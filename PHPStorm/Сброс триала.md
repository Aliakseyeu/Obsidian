...
### Для Windows:
```
Удалить папку C:\Users\...\AppData\Roaming\JetBrains\PhpStorm2020.1\eval

В реестре удалить HKEY_CURRENT_USER\Software\JavaSoft\Prefs\jetbrains\phpstorm

В файле удалить строки 
C:\Users\...\AppData\Roaming\JetBrains\PhpStorm2020.1\options\others.xml — не весь файл, иначе потом нужно будет настраивать окружение.
А только три строчки:
<property name=«evlsprt.201» value=»...» />
<property name=«evlsprt2.201» value=»...» />
<property name=«evlsprt3.201» value=»...» />
```

### Для MacOS:
```
Удалить файл /Users//Library/Preferences/PhpStorm/eval/PhpStorm.evaluation.key
Изменить файл /Users//Library/Preferences/PhpStorm/options/options.xml см ниже
```
...
Сделать батник либо взять готовый
...
![[reset-trial-jetbrains-windows.bat]]

Запустить от администратора
Запустить впн (например TunnelBear)
Запустить Шторм
Активировать триал

---

### Ubuntu (PhpStorm 2024.1.6)

- Открыть ~/.config/JetBrains/PhpStorm/options/other.xml
- Удалить строки начинающиеся с evlsprt
- Удалить папку ~/.java/.userPrefs

![[reset_phpstorm_trial 1.sh]]